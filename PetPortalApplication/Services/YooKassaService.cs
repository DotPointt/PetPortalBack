...
public class YooKassaService : IPaymentService //, IHostedService
{
    ...
    {
        var maxAttempts = 100; // Максимальное количество попыток (например, 5 минут при задержке 3 секунды)
        var attempt = 0;

        while (attempt < maxAttempts)
        {
            // Проверяем статус оплаты
            var payment = await _asyncClient.GetPaymentAsync(id);
            var isPaid = payment.Paid;
    ...
    // var url = payment.Confirmation.ConfirmationUrl;
}